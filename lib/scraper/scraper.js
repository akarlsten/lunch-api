...
const scrapeSlagt = async () => {
  ...
  await osmosis
    ...
    .set([osmosis.find('//*[@id="dagens-lunch"]/div/div[1]/center/p/text()[not(strong)]').set('dish')])
    .data(data => {
...
const scrapeGlasklart = async () => {
  ...
  await osmosis
    ...
    .data(data => {
      ...
      result.distance = distanceFromGleerups(result.position.lat, result.position.long)
      result.checkDay = weekday
      if (weekday < 6 && todaysDish && weeklyVeg) {
...
const scrapeNiagara = async () => {
  ...
  await osmosis
    ...
    .set([osmosis.find('body > section > div.row.row--narrow > div > div:nth-child(7) tr td:has(br)').set({ dish: 'text()[1]' })])
    .data(data => {
      result.distance = distanceFromGleerups(result.position.lat, result.position.long)
      result.checkDay = weekday
      switch (weekday) {
...
const scrapeValfarden = async () => {
  ...
  await osmosis
    ...
    .set([osmosis.find('.gmail_default p:not(strong)').set({ dish: 'text()' })])
    .data(data => {
      result.distance = distanceFromGleerups(result.position.lat, result.position.long)
      result.checkDay = weekday
      const noEmpty = data.filter(entries => entries.dish.length > 0)
      switch (weekday) {
      case '1': // Mon
        result.menuItems = noEmpty.slice(0, 2)
        break
      case '2': // Tue
        result.menuItems = noEmpty.slice(2, 4)
        break
      case '3': //Weds
        result.menuItems = noEmpty.slice(4, 6)
        break
      case '4': //Thurs
        result.menuItems = noEmpty.slice(6, 8)
        break
      case '5': //fri
        result.menuItems = noEmpty.slice(8, 11)
        break
      default:
        result.menuItems = []
      }
...
// above is when they sometimes run newsletters
/*
const scrapeValfarden = async () => {
  const result = {
    id: '8',
